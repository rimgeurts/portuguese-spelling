  ...
  return (
    <div className={"p-4"}>
      <ResultModal
        ...
        isCorrect={isCorrect}
        answer={answer}
        loadNextQuestion={handleLoadNextQuestion}
        openResultsModal={openResultsModal}
        correctAnswer={words[currentWordIndex].portguese}
      />
      <div className={"mx-auto max-w-7xl sm:px-6 lg:px-8"}>
        <div className={"flex justify-start"}>
          <div
            className={`sm:px-6 sm:py-3 px-3 py-1 rounded-md font-bold sm:text-2xl text-lg bg-pink-100 text-pink-600 border-2 border-pink-600`}
          >
            Score: {score} / {totalQuestionsNo}
          </div>
        </div>
